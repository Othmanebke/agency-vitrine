  ...
  return (
    <section id="testimonials" className="max-w-7xl mx-auto px-6 py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {items.map((it, idx) => (
          <motion.blockquote key={it.name} variants={card} initial="hidden" animate="visible" custom={idx+1} className="p-6 bg-zinc-900 rounded-lg shadow">
            <div className="flex items-center gap-4 mb-3">
              <div className="w-12 h-12 bg-zinc-800 rounded-full flex items-center justify-center text-sm">{it.name.split(' ')[0].charAt(0)}</div>
              <div>
